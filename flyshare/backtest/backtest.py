...
class Backtest():
    ...
    def __backtest_init(self):
       ...
       # 股票的交易日历,真实回测的交易周期,和交易周期在交易日历中的id
        ...
        self.account.init_assets = 1000000
    ...
    def __backtest_prepare(self):
        ...
        if len(str(self.strategy_start_date)) == 10:
            ...
            self.__backtest_log_info(self, 'Wrong start date format')
    ...
    def __backtest_before_backtest(self, *args, **kwargs):
        """
        这个是回测流程开始的入口
        """
    ...
    def __bid_amount(self, __strategy_amount, __amount):
    ...
    def __sync_order_LM(self, event_, order_=None, order_id_=None, trade_id_=None, market_message_=None):
        ...
        if event_ is 'init_':
            ...
            if len(self.account.hold) > 1:
                ...
                self.account_d_value.append(self.account.cash[-1] + sum([self.backtest_get_market_data_bar(
    ...
    def __wrap_bid(self, __bid, __order=None):
        __market_data_for_backtest = self.backtest_find_bar(
        ...
        if __market_data_for_backtest is not None:
            ...
            self.__backtest_log_info(self, 'BACKTEST ENGINE ERROR=== CODE %s TIME %s NO MARKET DATA!' % (
    ...
    def backtest_before_init(self):
        return self.__backtest_init()

    def backtest_after_init(self):
        return self.__backtest_prepare()

    @lru_cache()
    def backtest_find_bar(self, code, time):
    ...
    @lru_cache()
    def backtest_get_market_data(self, code, date, gap_=None, type_='lt'):
    ...
    @lru_cache()
    def backtest_get_market_data_panel(self, date=None, type_='lt'):
        try:
            if date is not None:
                if type_ in ['lt']:
    ...
    @lru_cache()
    def backtest_get_market_data_bar(self, code, time, if_trade=True):
        '这个函数封装了关于获取的方式'
        try:
    ...
    def backtest_get_block(self, block_list):
        block_=fetch_stock_block_adv()
        _data=[]
    ...
    #@lru_cache()
    def backtest_sell_available(self, __code):
        try:
   ...
   # @lru_cache()
    ...
    def backtest_hold_price(self, __code):
        try:
            return self.backtest_hold(self)['price'].groupby('code').mean()[__code]
        except:
            return None

    @lru_cache()
    def backtest_get_OHLCV(self, __data):
        '快速返回 OHLCV格式'
        return (__data.open, __data.high, __data.low, __data.close, __data.vol)

    def backtest_send_order(self, __code, __amount, __towards, __order):
        """
        2017/8/4
        委托函数
    ...
    @lru_cache()
    def backtest_check_order(self, order_id_):
        '用于检查委托单的状态'
        """
        ...
        for i in range(int(_cls.start_real_id), int(_cls.end_real_id)):
            ...
            if _cls.backtest_type in ['day', 'd', 'index_day']:
                ...
                if _cls.backtest_type in ['1min', 'index_1min']:
                    ...
                    type_ = '60min'
                daily_min = util_make_min_index(_cls.today, type_)  # 创造分钟线index
    ...
    def backtest_init(_cls, func, *arg, **kwargs):
        def __init_backtest(_cls, *arg, **kwargs):
            _cls.__backtest_init(_cls)
            func(*arg, **kwargs)
            _cls.__backtest_prepare(_cls)
    ...
    def before_backtest(_cls, func, *arg, **kwargs):
        def __before_backtest(_cls, *arg, **kwargs):
            func(*arg, **kwargs)
            _cls.__backtest_before_backtest(_cls)
