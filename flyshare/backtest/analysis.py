...
def backtest_analysis_start(client, code_list, assets_d, account_days, message, total_date, benchmark_data):
    ...
    # benchmark资产
    benchmark_assets = backtest_calc_benchmark(
        benchmark_data, assets[0])
    # d2=pd.concat([data.resample('D').last(),pd.DataFrame(benchmark_assets,columns=['benchmark'])])
    # benchmark年化收益
    benchmark_annualized_returns = backtest_calc_profit_per_year(
        benchmark_assets, len(total_date))
    # 计算账户的收益

    # days=len(assets_history)-1
    # 策略年化收益
    annualized_returns = backtest_calc_profit_per_year(
        assets_d, len(total_date))

    # 收益矩阵
    assets_profit = backtest_calc_profit_matrix(assets)
    benchmark_profit = backtest_calc_profit_matrix(benchmark_assets)

    # 策略日收益
    profit_day = backtest_calc_profit_matrix(assets_d)
    # 胜率
    win_rate = backtest_calc_win_rate(assets_profit)
    # 日胜率
    win_rate_day = backtest_calc_win_rate(profit_day)
    # 年化波动率
    volatility_year = backtest_calc_volatility(profit_day)
    benchmark_volatility_year = backtest_calc_volatility(benchmark_profit)
    # 夏普比率
    sharpe = backtest_calc_sharpe(
        annualized_returns, 0.05, volatility_year)

    # 最大回撤
    max_drop = backtest_calc_dropback_max(assets_d)

    # 计算beta
    beta = backtest_calc_beta(profit_day, benchmark_profit)
    # 计算Alpha
    alpha = backtest_calc_alpha(
...
def backtest_calc_assets(trade_history, assets):
    assets_d = []
    trade_date = []
...
def backtest_calc_alpha(annualized_returns, benchmark_annualized_returns, beta, r):
...
def backtest_calc_beta(assets_profit, benchmark_profit):
...
def backtest_calc_profit(assets_history):
    return (assets_history[-1] / assets_history[1]) - 1


def backtest_calc_profit_per_year(assets_history, days):
    return math.pow(float(assets_history[-1]) / float(assets_history[0]), 250.0 / float(days)) - 1.0


def backtest_calc_profit_matrix(assets_history):
    assets_profit = []
...
def backtest_calc_volatility(assets_profit_matrix):
    # 策略每日收益的年化标准差
...
def backtest_calc_dropback_max(history):
    drops = []
...
def backtest_calc_sharpe(annualized_returns, r, volatility_year):
    '计算夏普比率'
    return (annualized_returns - r) / volatility_year


def backtest_calc_trade_date(history):
    '计算交易日期'
    trade_date = []
...
def backtest_calc_win_rate(profit_day):
    # 大于0的次数
    abovez = 0
    belowz = 0
